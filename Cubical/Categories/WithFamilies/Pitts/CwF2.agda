...
module Categorical {ℓOb ℓHom : Level} (C : Category ℓOb ℓHom) where
  ...
  record CwF (ℓTy ℓTm : Level) :
    ...
    field
      ...
      [id]Tm : {A : Ty[ Γ ]} (a : Tm Γ A)
             → ------------------------------------------------
               PathP (λ i → Tm Γ (Ty .F-id i A)) (a [ id ]Tm) a

      [][]Tm : {A : Ty[ Γ ]} (a : Tm Γ A) (σ' : Θ ⟶ Δ) (σ : Δ ⟶ Γ)
             → ------------------------------------------------
                PathP (λ i → Tm Θ (Ty .F-seq σ σ' i A))
                      (a [ σ ∘ σ' ]Tm)
                      (a [ σ ]Tm [ σ' ]Tm)
    ...
    field
      ...
      ctxExtIsoFunNat : (A : Ty[ Γ ]) (σ : Δ ⟶ Γ ⋆ A) (τ : Θ ⟶ Δ)
                      → ctxExtIso A .fun (σ ∘ τ) ≡ (drop A σ ∘ τ , subst (Tm Θ) (sym (funExt⁻ (Ty .F-seq (drop A σ) τ) A)) ((ctxExtIso A .fun σ .snd) [ τ ]Tm))
      ...
      ΣTmIsoInvNat : (A : Ty[ Γ ])
                   ...
                   → PathP (λ i → Tm Δ (ΣTyNat A B σ i))
                           ...
                           (ΣTmIso (A [ σ ]Ty) (B [ ctxExt .F-hom (σ , refl) ]Ty) .inv
                             (a [ σ ]Tm , subst (Tm Δ) (coerceInv A B a σ) (b [ σ ]Tm)))
