-- TODO: add fst, snd, pairing with computation and naturality rules

-- {-# OPTIONS --safe --lossy-unification #-}
module Cubical.Categories.WithFamilies.Amy.CwFSigma where
...
module _ {ℓ ℓ' ℓ'' ℓ'''} (M : Motives ℓ ℓ' ℓ'' ℓ''') where
  ...
  record Methods : Type (ℓ-suc (ℓ ⊔ ℓ' ⊔ ℓ'' ⊔ ℓ''')) where
    field
      ...
      Σᴹ   : (Aᴹ : Tyᴹ Γᴹ A) (Bᴹ : Tyᴹ (Γᴹ ,ᴹ Aᴹ) B) → Tyᴹ Γᴹ (ΣTy A B)
      ...
      Σ[]ᴹ
        ...
        → PathP (λ i → Tyᴹ Γᴹ (ΣTy[] {A = A} {B = B} p i))
          ((Σᴹ Aᴹ Bᴹ) [ δᴹ ]Tᴹ)
    ...
    private
      ...
      Σ[]-case
        ...
        → PathP (λ i → Tyᴹ (Con-elim Γ) (ΣTy[] {A = A} {B = B} p i))
            (Ty-elim (ΣTy A B [ δ ]T)) (Ty-elim (ΣTy[] {A = A} {B = B} p i1))
    ...
    Ty-elim (ΣTy A B)      = Σᴹ (Ty-elim A) (Ty-elim B)
    Ty-elim (ΣTy[] {Γ = Γ} {A = A} {δ = δ} {B = B} {t₀ = t₀} x i) = Σ[]-case t₀ x i
    ...
    Σ[]-case {B = B} t₀ x i =
      Σ[]ᴹ {Bᴹ = Ty-elim B} {t₀ᴹ = Tm-elim t₀} {p = x} (λ j → Tm-elim (x j)) i
...
module Internal (U : Type ℓ)
  ...
  U-model .Σᴹ Aᴹ Bᴹ x = Sig (Aᴹ x) λ y → Bᴹ (SigIso _ _ .inv (x , y))
  U-model .Σ[]ᴹ {δᴹ = δᴹ} {Bᴹ = Bᴹ} h =
    funExt (λ x → cong (Sig _)
      (funExt (λ y → cong Bᴹ (cong (inv (SigIso _ _))
