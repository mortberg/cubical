...
Before we can talk about the recursor and induction principle we need
the following auxiliary definitions (all defined using J):

- Given f : A → B and p : Id A x y we have cong f p : Id B (f x) (f y).
- Given P : A → U and p : Id A x y we have subst P p : P x → P y.
- Given P : A → U, f : (x : A) → P x and p : Id A x y, we have
...
The dependent induction principle is a bit more complicated. Given
P : S¹ → U with
...
*native* support for HITs where things compute better and are simpler
(no subst/cong/congD). Cubical type theory is such a type theory.
...
Voevodsky's univalence axiom, but we have also found that it is a
suitable system for well-behaved higher inductive types. The original
...
In this paper we sketched how to extend CTT with spheres and
propositional truncation HITs.
...
should be thought of as a line between A(0) and A(1). We can bind
...
us define path-over types directly. Given A : I → U, x : A 0 and y : A 1
we define:
...
constructor of S¹ and not its Id/Path type (compare this to for
example "suc : ℕ → ℕ" or "cons : A → List A → List A").
...
The dependent induction principle is not more complicated. Given
P : S¹ → U with
